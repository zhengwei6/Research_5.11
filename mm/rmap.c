...
int __anon_vma_prepare(struct vm_area_struct *vma)
{
	...
	if (likely(!vma->anon_vma)) {
		...
		if (mm->owner != NULL && mm->owner->dl.dl_runtime > 0) {
			...
			anon_vma->pin_page_list = &mm->owner->dl.pin_page_list;
		}
		else {
				...
				if (anon_vma != NULL) {
					...
					if (root != NULL) {
						...
						if (pgdat != NULL) {
							...
							if (refault_anon_shadow != NULL && lruvec != NULL) {
								refault_anon_shadow->inactive_age_shadow = atomic_long_read(&lruvec->inactive_age_anon);
